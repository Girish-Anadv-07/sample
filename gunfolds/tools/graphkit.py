...
from gunfolds.tools.conversions import nx2graph, nxbp2graph, graph2adj, graph2nx
...
def mean_degree_graph(node_num, degree):
    """
    Returns mean outgoing degree of the graph
...
def bp_mean_degree_graph(node_num, degree, seed=None):
    ...
    :returns: a random graph with ``node_num``, nodes and mean outgoing degree of ``degree``
    :rtype: dictionary(``gunfolds`` graph)
    """
