    ...
    for v in g:
        for w in g[v]:
            if both_bidirected:
                ...
                direction = v < w
            if g[v][w] & 1: s += directed+'('+str(v)+','+str(w)+'). '
            if g[v][w] & 2 and direction: s += bidirected+'('+str(v)+','+str(w)+'). '
    return s
...
def numbered_g2clingo(g, n, directed='hdirected', bidirected='hbidirected'):
    ...
    for v in g:
        for w in g[v]:
            if g[v][w] & 1: s += directed+'('+str(v)+','+str(w)+','+str(n)+'). '
            if g[v][w] & 2 and v < w: s += bidirected+'('+str(v)+','+str(w)+','+str(n)+'). '
    return s
