...
from gunfolds.tools.conversions import g2clingo, rate, rasl_jclingo2g,\
...
def rasl_command(g, urate=0, all_u=True):
    ...
    :param all_u: (Ask its not used in the program)
    :type all_u: boolean
...
def drasl_command(g_list, max_urate=0, weighted=False, scc=False, dm=None, bdm=None, edge_weights=(1,1)):
    ...
    :param edge_weights: a tuple of 2 values, the first is importance of matching directed weights when solving optimization problem and the second is for bidirected.
    ...
    if scc: command += encode_list_sccs(g_list)
